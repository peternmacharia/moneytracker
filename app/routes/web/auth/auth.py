...
def enforce_password_change():
    """
    Any authenticated user who has never changed their autogenerated password is redirected to the change-password page,
...
def get_post_login_redirect(user):
    ...
    if user.has_role("super", "admin"):
        return url_for("admin_main.dashboard")
    elif user.has_role("user"):
        return url_for("public_main.dashboard")
    else:
...
def verify_email(token):
    ...
    if user.password_changed_at is not None:
        flash("Your email has already been verified. Please log in, or use 'Forgot password' if needed.", "info")
    ...
    if form.validate_on_submit():
        ...
        current_app.logger.info("Email verified and password set successfully | user_id=%s | ip=%s", user.id, request.remote_addr)
        flash("Your email has been verified and password set. You can now log in.", "success")
        return redirect(url_for("auth.login"))
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        user.set_password(secrets.token_urlsafe(32))  # Generate a random password for the user
        db.session.add(user)
        db.session.flush()
